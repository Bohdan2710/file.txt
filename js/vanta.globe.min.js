  ...
  h.VantaBase = class {
    constructor() {
      ...
      if (this.options = Object.assign({
        ...
        el: t
      }), Object.assign(this.options, t), this.options.THREE && (r = this.options.THREE), this.el = this.options.el, null == this.el) a('Instance needs "el" param!');else if (!(this.options.el instanceof HTMLElement)) {
        const t = this.el;
      ...
      try {
        ...
        return a("Init error", t), this.renderer && this.renderer.domElement && this.el.removeChild(this.renderer.domElement), void (this.options.backgroundColor && (console.log("[VANTA] Falling back to backgroundColor"), this.el.style.background = (i = this.options.backgroundColor, "number" == typeof i ? "#" + ("00000" + i.toString(16)).slice(-6) : i)));
      }
    ...
    windowMouseMoveWrapper(t) {
      ...
      const s = t.clientX - e.left,
        i = t.clientY - e.top;
      s >= 0 && i >= 0 && s <= e.width && i <= e.height && (this.mouseX = s, this.mouseY = i, this.options.mouseEase || this.triggerMouseMove(s, i));
    }
    ...
    destroy() {
      ...
      const e = this.scene;
      e && e.children && o(e), this.renderer && (this.renderer.domElement && this.el.removeChild(this.renderer.domElement), this.renderer = null, this.scene = null), h.current === this && (h.current = null);
    }
  };
  const l = h.VantaBase,
  ...
  class d extends l {
    ...
    genPoint(t, e, s) {
      ...
      if (this.points || (this.points = []), this.options.showDots) {
        const t = new p.SphereGeometry(.25, 12, 12),
          e = new p.MeshLambertMaterial({
            color: this.options.color
          });
        i = new p.Mesh(t, e);
      } else i = new p.Object3D();
      return this.cont.add(i), i.ox = t, i.oy = e, i.oz = s, i.position.set(t, e, s), i.r = 0, this.points.push(i);
    }
    onInit() {
      ...
      const o = t * t * 2;
      this.linePositions = new Float32Array(o * o * 3), this.lineColors = new Float32Array(o * o * 3);
      ...
      h.setAttribute("position", new p.BufferAttribute(this.linePositions, 3).setUsage(p.DynamicDrawUsage)), h.setAttribute("color", new p.BufferAttribute(this.lineColors, 3).setUsage(p.DynamicDrawUsage)), h.computeBoundingSphere(), h.setDrawRange(0, 0);
      const a = new p.LineBasicMaterial({
        vertexColors: p.VertexColors,
        blending: "additive" === this.blending ? p.AdditiveBlending : null,
        transparent: !0
      });
      this.linesMesh = new p.LineSegments(h, a), this.cont.add(this.linesMesh);
      ...
      this.camera = new p.PerspectiveCamera(20, this.width / this.height, .01, 1e4), this.camera.position.set(50, 100, 150), this.scene.add(this.camera);
      const l = new p.AmbientLight(16777215, .75);
      this.scene.add(l), this.spot = new p.SpotLight(16777215, 1), this.spot.position.set(0, 200, 0), this.spot.distance = 400, this.spot.target = this.cont, this.scene.add(this.spot), this.cont2 = new p.Group(), this.cont2.position.set(0, 15, 0), this.scene.add(this.cont2);
      const c = new p.LineBasicMaterial({
      ...
      for (let t = 0; t < 80; t++) {
        const t = s(18, 24),
          ...
          r = Math.sin(n) * o,
          h = Math.cos(n) * o;
        d.push(new p.Vector3(h * t, r * t, i * t)), d.push(new p.Vector3(h * e, r * e, i * e));
      }
      const u = new p.BufferGeometry().setFromPoints(d);
      this.linesMesh2 = new p.LineSegments(u, c), this.linesMesh2.position.set(0, 0, 0), this.cont2.add(this.linesMesh2);
      const m = new p.LineBasicMaterial({
          color: this.options.color2,
          linewidth: 2
        }),
        w = [];
      ...
      const g = new p.LineBasicMaterial({
          color: this.options.color
        }),
    ...
    onUpdate() {
      ...
      for (let t = 0; t < this.points.length; t++) {
        ...
        l.scale.z = (.25 * (15 - c)).clamp(1, 100), l.scale.x = l.scale.y = l.scale.z, l.position.y = 2 * Math.sin(l.position.x / 10 + .01 * this.t + l.position.z / 10 * .5);
    ...
    onMouseMove(t, e) {
      ...
      s.tz = o * Math.sin(n), s.tx = o * Math.cos(n), s.ty = s.oy + 80 * (e - .5) * (this.options.mouseCoeffY || 1), this.rayCaster, this.rcMouseX = 2 * t - 1, this.rcMouseY = 2 * -t + 1;
    }
    onRestart() {
